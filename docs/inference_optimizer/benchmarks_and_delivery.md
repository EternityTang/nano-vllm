...
### P6c：B3/B4/B5 profile and stabilization

P6c 不是新的 optimizer feature，也不是新的 headline ablation 组；它是 B3/B4/B5 的 profiling/stabilization pass，用来解释 kernel microbench 与 serving goodput 的差距。

| 项 | 内容 |
|---|---|
| 输入命令 | B3/B4/B5 原命令，输出到 `results/b3_profile.json`、`results/b4_profile.json`、`results/b5_profile.json` |
| 禁止事项 | 不开启 EVICT，不新增 optimizer 行为，不进入 P7 release 改写 |
| 对应验证问题 | B5 是否真的 dispatch fused kernel？fallback / unsupported shape / parity check / metadata packing 是否吞掉收益？ |
| profile metrics | `avg_step_ms`、`fused_kernel_calls`、`fused_kernel_fallbacks`、`fallback_count`、`visible_table_tensor_pack_ms`、`gather_dequant_ms`、`mixed_kv_decode_kernel_ms`、`model_forward_non_attention_ms` |
| 当前结论 | P6c 后 B5 `fused_kernel_calls=15512`、`fused_kernel_fallbacks=0`、`fallback_count=0`，goodput 从旧 B5 `10.02 tok/s` 提升到 `38.64 tok/s` |
| 是否进入 headline | 作为解释性 profiling 附录；不替代 B0-B5 ablation |
